...
function buildCapacity(ctx, Po, phiCap, capFac, spiral, shape) {
  ...
  const Ptop = phi0 * Po;
  const curveX = phiCurve(Math.PI / 2, ctx, spiral, "Mx");
  const curveY = shape === "rect" ? phiCurve(0, ctx, spiral, "My") : curveX;
  let contours = null;
  if (shape === "rect") {
    contours = []; const NP = 26;
    for (let i = 0; i <= NP; i++) { const Pl = Ptop * i / NP; contours.push({ P: Pl, poly: phiContourAt(Pl, ctx, spiral, 48) }); }
  }
  return { Pmax, Po, Ptop, curveX, curveY, contours, shape, ety: ctx.fy / ctx.Es };
}
...
function dcrRadial(cap, Pu, Mux, Muy, biaxial) {
  ...
  const Rcap = (Pq) => biaxial ? contourRadiusAt(cap, Pq, psi) : curveMomentAt(aboutX ? cap.curveX : cap.curveY, Pq);
  ...
  if (biaxial && cap.contours) {
    ...
    const cv = aboutX ? cap.curveX : cap.curveY;
  ...
  const api = {
    ...
    build() {
      ...
      for (const pg of pages) {
        ...
        pageNums.push(add(`<< /Type /Page /Parent ${reserved} 0 R /MediaBox [0 0 ${api.W.toFixed(2)} ${api.H.toFixed(2)}] /Resources << /Font << /F1 ${fontReg} 0 R /F2 ${fontBold} 0 R >> >> /Contents ${contentNum} 0 R >>`));
      }
...
function buildReport(S, FP, meta) {
  ...
  d.newPage(); y = 64;
  d.text(M, y, meta.app, { bold: true, size: 22 }); y += 24;
  d.text(M, y, "Reinforced-Concrete Column - Interaction Hand Calculations", { size: 12, color: [0.3, 0.34, 0.4] }); y += 16;
  d.text(M, y, "Five control points, derived by strain compatibility and verified against the app engine.", { size: 9.5, color: [0.45, 0.5, 0.55] }); y += 14;
  d.hline(M, colR, y, { width: 1.2, color: [0.11, 0.3, 0.85] }); y += 20;
  ...
  const detail = (p, idx) => {
    d.newPage(); y = 60; head();
    d.text(M, y, `${idx + 3}.  ${p.title}`, { bold: true, size: 12.5 }); y += 20;
    if (p.pureAxial) {
...
function ColumnCheck() {
  ...
  const R = useMemo(() => {
    ...
    return { S, res, gov, nominal, design, points, aboutX, depthUsed, isStrong, anyBiax, surface, surfPts, curves3D, soRes, rho, rhoWarn, spc, plan, axisLabel: S.shape === "circ" ? "" : `bending about ${aboutX ? "X" : "Y"} · depth ${aboutX ? "h" : "b"} = ${fmt(depthUsed, 0)} ${U.len}${isStrong === null ? "" : isStrong ? " (strong axis)" : " (weak axis)"}` };
  ...
  function doExportPDF() {
    try {
      ...
      const meta = {
        ...
        U: { len: U.len, stress: U.stress, force: U.force, moment: U.moment, forceDiv: U.forceDiv, momentDiv: U.momentDiv },
      ...
      setExMsg({ t: "ok", m: "Exported verification PDF — 7 pages of hand calculations at the five control points for " + short + "." });
    } catch (e) {
